...
// Cleans up the features by dropping truthy suffixes like ": 1" or ": true"
    ...
    if (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) {
      for (const [key, value] of Object.entries(parsed)) {
        if (Array.isArray(value)) {
          // If it's already an array of strings, just keep it clean
          normalized[key] = value.map(item => String(item));
        } else if (value && typeof value === 'object') {
          // It's an object: {"Anti-Theft & Locks": {"Child Safety Door Locks": 1}}
          normalized[key] = Object.entries(value).map(([subKey, subVal]) => {
            // If the value is a standard truthy flag (1, "1", true), just return the feature title
            if (subVal === 1 || subVal === "1" || subVal === true) {
              return subKey;
            }
            // If it's explicitly false or 0, return a readable negative state or omit it
            if (subVal === 0 || subVal === "0" || subVal === false) {
              return `No ${subKey}`;
            }
            // Fallback for actual variable descriptions (e.g., {"Engine": "V6"})
            return `${subKey}: ${String(subVal)}`;
          });
        } else {
          // If the top level property value itself is 1 or true
        ...
        return (
          <div key={key} className="border border-gray-200 rounded overflow-hidden shadow-sm">
            <button
              ...
              className="w-full bg-[#00a651] text-white font-semibold text-[14px] px-4 py-3 flex items-center justify-between"
            >
            ...
            {isOpen && (
              <div className="bg-white px-5 py-4">
                {!hasContent ? (
                  <p className="text-gray-400 text-[12px] italic py-2">No options listed for this category.</p>
                ) : (
