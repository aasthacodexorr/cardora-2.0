...
const Header = () => {
  ...
  const headerBgClass = isLandingPage && !isScrolled ? "bg-transparent" : "bg-white shadow-[0_2px_10px_rgba(0,0,0,0.05)]";
  const mobileHeaderBgClass = isLandingPage && !isScrolled ? "bg-transparent" : "bg-white shadow-sm";
  const topBarBgClass = isLandingPage && !isScrolled ? "bg-transparent" : "bg-white";

  return (
    <>
      {/* Desktop Header */}
      <header className={`hidden lg:block w-full sticky top-0 z-50 global_header transition-colors duration-300 ${headerBgClass}`}>
        <div className="mx-auto flex max-w-[1400px] items-center justify-between  py-4">
          ...
          <div className="flex-[0.2] flex justify-end">
            <a
              ...
              className="flex items-center gap-2 bg-brand-green text-brand-green-foreground px-6 py-3 rounded-full font-bold text-[15px] hover:opacity-90 transition-opacity whitespace-nowrap shadow-sm"
            >
      ...
      {/* Mobile Header */}
      <header className={`lg:hidden w-full sticky top-0 z-50 flex flex-col transition-colors duration-300 ${mobileHeaderBgClass}`}>
        {/* Top Bar */}
        <div className={`flex items-center justify-between px-4 py-3 z-50 transition-colors duration-300 ${topBarBgClass}`}>
          <Link href="/" aria-label="Cardora home" className="relative z-50">
            <CardoraLogo />
          </Link>
          <div className="flex items-center gap-5 relative z-50">
            <a href="https://facebook.com" target="_blank" rel="noreferrer" aria-label="Facebook" className="text-gray-600 hover:text-brand-green">
              <FacebookIcon className="h-[22px] w-[22px]" />
            </a>
            <a href="https://instagram.com" target="_blank" rel="noreferrer" aria-label="Instagram" className="text-gray-600 hover:text-brand-green">
        ...
        <div className="bg-brand-green text-white flex items-center justify-between px-6 py-2.5 z-50 shadow-md">
          <div className="flex items-center gap-8">
            <a href="tel:1-855-514-5500" className="flex flex-col items-center justify-center gap-1 group">
            ...
            <a href={SITE_CONFIG.urls.googleMapsUrl} target="_blank" rel="noreferrer" className="flex flex-col items-center justify-center gap-1 group">
